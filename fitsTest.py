...
assert(r.status_code == 200)
assert(len(r.json['types']) == 4)
#Adding back the recently deleted Internet Type
r = client.put('/type/5', json = {"name" : "Internet"})
...
r = client.get('/type')
assert(r.status_code == 200)
assert(len(r.json['types']) == 6)
assert(r.json['types'][5]['name'] == "Network")
r = client.post('/type')
assert(r.status_code == 403)
assert(r.json['error'] == "must provide a name field")
...
r = client.put('/issue/21', json = {'username': "millerl19", 'type' : "Internet", 'description' : "Cannot connect to Internet", 'location' : "Crowe Hall"})
assert(r.status_code == 201)
...
assert(r.json['location'] == "Crowe Hall")
...
r = client.post('/issue', json = {"username" : "millerl19", "type" : "Internet", 'description':"Speeds are very slow", 'location' : "Crowe Hall"})
assert(r.status_code == 201)
assert(r.json['ok'] == "issue created")
#Testing the issue_delete function
r = client.delete('/issue/22')
assert(r.status_code == 204)
r = client.get('/issue/22')
assert(r.status_code == 404)
assert(r.json['error'] == "issueId not found")
r = client.delete('/issue/345')
assert(r.status_code == 404)
assert(r.json['error'] == "issueId not found")
db.commit()
#Tesing the user_issues function
r = client.post('/issue', json = {"username" : "davish20", "type" : "Internet", 'description':"Speeds are very slow", 'location' : "Crowe Hall"})
r = client.get('/issue/user/millerl19')
assert(r.status_code == 200)
assert(len(r.json['issues']) == 2)
assert(r.json['issues'][0]['description'] == "Cannot connect to Internet")
assert(r.json['issues'][1]['description'] == "Speeds are very slow")
r = client.get('issue/user/hedrickj19')
assert(r.status_code == 404)
assert(r.json['error'] == "username not found in the database")
#Testing the type_issues function
r = client.get('/issue/type/Internet')
assert(r.status_code == 200)
assert(len(r.json['issues']) == 3)
assert(r.json['issues'][0]['userId'] == "millerl19")
assert(r.json['issues'][0]['description'] == "Cannot connect to Internet")
assert(r.json['issues'][0]['location'] == "Crowe Hall")
r = client.get('issue/type/Gaming')
assert(r.status_code == 404)
r = client.get('issue/type/Phone')
assert(r.status_code == 200)
assert(len(r.json['issues']) == 1)
assert(r.json['issues'][0]['userId'] == "monnine20")
#Testing the location_issues function
r = client.get('issue/location/Crowe Hall')
assert(r.status_code == 200)
assert(len(r.json['issues']) == 3)
assert(r.json['issues'][0]['id'] == 21)
assert(r.json['issues'][0]['location'] == "Crowe Hall")
assert(r.json['issues'][0]['userId'] == "millerl19")
assert(r.json['issues'][0]['type'] == "Internet")
assert(r.json['issues'][0]['description'] == "Cannot connect to Internet")
r = client.get('issue/location/Franklin')
assert(r.status_code == 404)
db.commit()
